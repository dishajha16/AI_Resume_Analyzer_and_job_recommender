...
class resume_analyzer:
    ...
    def Cohere(cohere_api_key, chunks, analyze):

        # Using cohere service for embedding
        embeddings = CohereEmbeddings(model="embed-english-v3.0", cohere_api_key=cohere_api_key, user_agent="langchain:partner")
        ...
        # creates a Cohere chat model
        llm = ChatCohere(cohere_api_key=cohere_api_key, model="command-r", user_agent="langchain:partner")
        ...
        prompt = (
            "You are a helpful assistant for resume analysis. "
    ...
    def resume_summary():
        ...
        if submit:
            if pdf is not None and cohere_api_key != '':
                try:
                    with st.spinner('Processing...'):
                        ...
                        summary = resume_analyzer.Cohere(cohere_api_key=cohere_api_key, chunks=pdf_chunks, analyze=summary_prompt)
                ...
                st.markdown(f'<h5 style="text-align: center;color: orange;">Please Enter cohere API Key</h5>', unsafe_allow_html=True)
            ...
            if pdf is not None and cohere_api_key != '':
                try:
                    with st.spinner('Processing...'):
                        ...
                        strength = resume_analyzer.Cohere(cohere_api_key=cohere_api_key, chunks=pdf_chunks, analyze=strength_prompt)
            ...
            if pdf is not None and cohere_api_key != '':
                try:
                    with st.spinner('Processing...'):
                        ...
                        weakness = resume_analyzer.Cohere(cohere_api_key=cohere_api_key, chunks=pdf_chunks, analyze=weakness_prompt)
            ...
            if pdf is not None and cohere_api_key != '':
                try:
                    with st.spinner('Processing...'):
                        ...
                        job_title = resume_analyzer.Cohere(cohere_api_key=cohere_api_key, chunks=pdf_chunks, analyze=job_title_prompt)
